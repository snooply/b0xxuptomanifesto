 ...
 *
 * pinOut by Danny, setup by simplecontrollers, loop and x and y dolphin values by Snapple, x and y values GC stick values by Hax$ and team
 */
...
void loop()
{
  ...
  if(mod1){
    ...
    if(cDown && ((leftOne ||rightOne) && (upOne || downOne))){
      pinxAxis = 128 + ((rightOne - leftOne)*127);
      pinyAxis = 128 + ((upOne - downOne)*99);
    }
    if(cLeft && ((leftOne ||rightOne) && (upOne || downOne))){
      pinxAxis = 128 + ((rightOne - leftOne)*127);
      pinyAxis = 128 + ((upOne - downOne)*78);
    }
    if(cRight && ((leftOne ||rightOne) && (upOne || downOne))){
      pinxAxis = 128 + ((rightOne - leftOne)*127);
      pinyAxis = 128 + ((upOne - downOne)*39);
    }
  ...
  if(mod2){
    ...
    //FireFox Angles with cButtons
    if(cUp && ((leftOne ||rightOne) && (upOne || downOne))){
      pinxAxis = 128 + ((rightOne - leftOne)*57);
      pinyAxis = 128 + ((upOne - downOne)*127);
    }
    if(cDown && ((leftOne ||rightOne) && (upOne || downOne))){
      pinxAxis = 128 + ((rightOne - leftOne)*99);
      pinyAxis = 128 + ((upOne - downOne)*127);
    }
    if(cLeft && ((leftOne ||rightOne) && (upOne || downOne))){
      pinxAxis = 128 + ((rightOne - leftOne)*78);
      pinyAxis = 128 + ((upOne - downOne)*127);
    }
    if(cRight && ((leftOne ||rightOne) && (upOne || downOne))){
      pinxAxisC = 128 + ((rightOne - leftOne)*39);
      pinyAxisC = 128 + ((upOne - downOne)*127);
    }
  }
